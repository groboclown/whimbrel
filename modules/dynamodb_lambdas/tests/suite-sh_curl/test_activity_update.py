...
def run_command(activity_exec_id, transition, source):
    config = get_config()
    ...
    host = m.group(1)
    exec_file = os.path.join(os.path.dirname(sys.argv[0]), '..', 'node.js', 'activity-update.sh')
    cmd = [exec_file, config['dynamodb']['db prefix'], endpoint, host, activity_exec_id, transition, source]
    env = dict(os.environ)
...
if __name__ == '__main__':
    run_command("activity1::12-23-423-cd", "start", "test")
